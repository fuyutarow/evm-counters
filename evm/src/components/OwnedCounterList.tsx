...
import { useCounter, useOwnedCounterList } from "@/hooks/useCounter";

export function OwnedCounterList() {
  const { data: ownedCounters = [], isLoading } = useOwnedCounterList();
  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
      {ownedCounters.map((counter) => (
        <OwnedCounterCard key={counter.id.toString()} counter={counter} />
      ))}
    </div>
  );
}

function OwnedCounterCard({ counter }: { counter: { id: bigint; value?: bigint } }) {
  const router = useRouter();
  const counterHook = useCounter();
  const valueQuery = counterHook.owned.useValue(counter.id);
      ...
      <CardContent className="pt-0">
        <div className="flex items-center justify-between">
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => router.push(`/owned-counter/${counterId}`)}
